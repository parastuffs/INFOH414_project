...
stepsInRoom = 0
STEPS_UNTIL_LEAVE = 150
STEPS_UNTIL_LEAVE_BEST_ROOM = 800
...
function step()
	...
	if inRoom == 1 and (not roomTransition) and (isMovingTowardRoom == 0) then
		roomQuality = senseRoomQuality()
	elseif inCentralRoom and (not roomTransition) then
